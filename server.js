...
async function calculatingCrossCurrency(call, callback) {
  ...
  let currencyInput = await getOneElementFromList(
    call.request.getSymbolInput()
  );

  let currencyOutput = await getOneElementFromList(
    call.request.getSymbolOutput()
  );

  let result =
    (currencyOutput.getRate() / currencyInput.getRate()) *
